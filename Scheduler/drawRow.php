    ...
    function drawRowPersonal()
    {
        ...
        { 
            ...
            while($row = mysqli_fetch_array($result))
            {
				...
				$username = $row['username'];
				$colorR=205-ord($username);//this should be similar to modR in drawRow()
				if ($colorR<0) $colorR=0;
				$colorB=193-((13 * strlen($username)) % 31);//this should be similar to modB in drawRow()
